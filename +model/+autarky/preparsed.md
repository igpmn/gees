% [IrisToolbox] autogenerated preparsed model source code 05-Sep-2023 17:18:59
%
...
!equations(:production)
    ...
    py1 = gamma_y2*py2 + gamma_mq*pq !! y1 = a_y1 * (y2/(1-gamma_q))^(1-gamma_q) * (mq/gamma_q)^gamma_q ;

    gamma_y2 * y1 = y2;

    gamma_mq * y1 = mq;

    gamma_y2 = + rho_gamma * gamma_y2{-1} + (1 - rho_gamma) * (1-gamma_q)*(&py1/&py2) !! gamma_y2 = (1-gamma_q)*(py1/py2) ;

    gamma_mq = + rho_gamma * gamma_mq{-1} + (1 - rho_gamma) * gamma_q*(&py1/&pq) !! gamma_mq = gamma_q*(py1/pq) ;
