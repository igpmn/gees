% [IrisToolbox] autogenerated preparsed model source code 22-Apr-2022 10:32:19
%
...
!equations(:production)
    ...
    py1 = py2^(1 - gamma_q) * pq^gamma_q !! y1 = (y2/(1-gamma_q))^(1-gamma_q) * (mq/gamma_q)^gamma_q;


    (1-gamma_q) * &py1 * y1 = &py2 * y2 !! (1-gamma_q) * py1 * y1 = py2 * y2;


    (gamma_q) * &py1 * y1 = &pq * mq !! gamma_q * py1 * y1 = pq * mq;
...
!equations(:monetary)
    ...
    react = + psi_pc * (log(roc_pc{+1}) - log(ss_roc_pc)) + psi_e * (log(roc_e) - log(&roc_e)) + psi_nh * (log(nh) - log(&nh)) !! react = 0;
