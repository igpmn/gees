% [IrisToolbox] autogenerated preparsed model source code 22-Apr-2022 10:32:24
%
...
!variables(:us_households)

    "Private consumption" us_ch
    "Per-capita private consumption" us_ch_to_nn
    "Relative point of reference for private consumption" us_ref_ch_to_ch
...
!parameters(:us_households :us_steady)
...
!parameters(:us_households :us_transitory)

    "A/R in real wage rate" us_rho_w
    "A/R in uncertainty discount factor on capital" us_rho_zk
    "A/R in uncertainty discount factor on production cash flows" us_rho_zy
...
!equations(:us_households)


    "Optimal choice of household consumption"
    us_vh*us_pc*(us_ch - us_chi*us_ref_ch_to_ch*us_ch) = us_nn*(1 - us_chi*&us_ref_ch_to_ch) !! us_vh*us_pc*us_ch = us_nn;
...
!parameters(:us_production :us_steady)
...
!parameters(:us_production :us_transitory)
...
!parameters(:us_monetary :us_transitory)
    ...
    "Monetary policy reaction to real economic activity" us_psi_nh
...
!equations(:us_monetary)
    ...
    us_react = + us_psi_pc * (log(us_roc_pc{+1}) - log(us_ss_roc_pc)) + us_psi_e * (log(us_roc_e) - log(&us_roc_e)) + us_psi_nh * (log(us_nh) - log(&us_nh)) !! us_react = 0;
...
!parameters(:us_open :us_steady)
    ...
    "S/S Autonomous component in country credit risk" us_ss_zh_aut


!parameters(:us_open :us_transition)

    "A/R Autonomous component in country credit risk" us_rho_zh_aut
...
!variables(:ea_households)

    "Private consumption" ea_ch
    "Per-capita private consumption" ea_ch_to_nn
    "Relative point of reference for private consumption" ea_ref_ch_to_ch
...
!parameters(:ea_households :ea_steady)
...
!parameters(:ea_households :ea_transitory)

    "A/R in real wage rate" ea_rho_w
    "A/R in uncertainty discount factor on capital" ea_rho_zk
    "A/R in uncertainty discount factor on production cash flows" ea_rho_zy
    "Point of reference in consuptions switch" ea_chi
...
!equations(:ea_households)


    "Optimal choice of household consumption"
    ea_vh*ea_pc*(ea_ch - ea_chi*ea_ref_ch_to_ch*ea_ch) = ea_nn*(1 - ea_chi*&ea_ref_ch_to_ch) !! ea_vh*ea_pc*ea_ch = ea_nn;
...
!parameters(:ea_production :ea_steady)
...
!parameters(:ea_production :ea_transitory)
...
!parameters(:ea_monetary :ea_transitory)
    ...
    "Monetary policy reaction to real economic activity" ea_psi_nh
...
!equations(:ea_monetary)
    ...
    "Monetary policy reaction term"
    ea_react = + ea_psi_pc * (log(ea_roc_pc{+1}) - log(ea_ss_roc_pc)) + ea_psi_e * (log(ea_roc_e) - log(&ea_roc_e)) + ea_psi_nh * (log(ea_nh) - log(&ea_nh)) !! ea_react = 0;
...
!parameters(:ea_open :ea_steady)
    ...
    "S/S Autonomous component in country credit risk" ea_ss_zh_aut


!parameters(:ea_open :ea_transition)

    "A/R Autonomous component in country credit risk" ea_rho_zh_aut
...
!variables(:cn_households)

    "Private consumption" cn_ch
    "Per-capita private consumption" cn_ch_to_nn
    "Relative point of reference for private consumption" cn_ref_ch_to_ch
...
!parameters(:cn_households :cn_steady)
...
!parameters(:cn_households :cn_transitory)

    "A/R in real wage rate" cn_rho_w
    "A/R in uncertainty discount factor on capital" cn_rho_zk
    "A/R in uncertainty discount factor on production cash flows" cn_rho_zy
...
!equations(:cn_households)


    "Optimal choice of household consumption"
    cn_vh*cn_pc*(cn_ch - cn_chi*cn_ref_ch_to_ch*cn_ch) = cn_nn*(1 - cn_chi*&cn_ref_ch_to_ch) !! cn_vh*cn_pc*cn_ch = cn_nn;
...
!parameters(:cn_production :cn_steady)
...
!parameters(:cn_production :cn_transitory)
...
!parameters(:cn_monetary :cn_transitory)
    ...
    "Monetary policy reaction to real economic activity" cn_psi_nh
...
!equations(:cn_monetary)
    ...
    "Monetary policy reaction term"
    cn_react = + cn_psi_pc * (log(cn_roc_pc{+1}) - log(cn_ss_roc_pc)) + cn_psi_e * (log(cn_roc_e) - log(&cn_roc_e)) + cn_psi_nh * (log(cn_nh) - log(&cn_nh)) !! cn_react = 0;
...
!parameters(:cn_open :cn_steady)
    ...
    "S/S Autonomous component in country credit risk" cn_ss_zh_aut


!parameters(:cn_open :cn_transition)

    "A/R Autonomous component in country credit risk" cn_rho_zh_aut
...
!variables(:rw_households)

    "Private consumption" rw_ch
    "Per-capita private consumption" rw_ch_to_nn
    "Relative point of reference for private consumption" rw_ref_ch_to_ch
...
!parameters(:rw_households :rw_steady)
...
!parameters(:rw_households :rw_transitory)

    "A/R in real wage rate" rw_rho_w
    "A/R in uncertainty discount factor on capital" rw_rho_zk
    "A/R in uncertainty discount factor on production cash flows" rw_rho_zy
...
!equations(:rw_households)


    "Optimal choice of household consumption"
    rw_vh*rw_pc*(rw_ch - rw_chi*rw_ref_ch_to_ch*rw_ch) = rw_nn*(1 - rw_chi*&rw_ref_ch_to_ch) !! rw_vh*rw_pc*rw_ch = rw_nn;
...
!parameters(:rw_production :rw_steady)
...
!parameters(:rw_production :rw_transitory)
...
!parameters(:rw_monetary :rw_steady)

    "S/S Price of private consumption, Rate of change" rw_ss_roc_pc
    "Switch for lower bound on short-term rate" rw_floor


!parameters(:rw_monetary :rw_transitory)

    "A/R in policy rate" rw_rho_r 
    "Monetary policy reaction to CPI" rw_psi_pc 
    "Monetary policy reaction to nominal exchange rate" rw_psi_e
    "Monetary policy reaction to real economic activity" rw_psi_nh
...
!equations(:rw_monetary)
    ...
    "Monetary policy reaction term"
    rw_react = + rw_psi_pc * (log(rw_roc_pc{+1}) - log(rw_ss_roc_pc)) + rw_psi_e * (log(rw_roc_e) - log(&rw_roc_e)) + rw_psi_nh * (log(rw_nh) - log(&rw_nh)) !! rw_react = 0;


    "Zero floor constraint on policy rate"
    rw_r = rw_floor*max(1, rw_unc_r) + (1-rw_floor)*rw_unc_r;
...
!variables
    ...
    "Net lump-sum tax on wealthy consumers to consumption ratio" rw_txls1_to_nc
    "Net lump-sum tax on poor consumers to consumption ratio" rw_txls2_to_nc
    "Government consumption, Rate of Change" rw_roc_cg
...
!parameters
    ...
    "S/S Government debt to GDP ratio" rw_ss_dg_to_ngdp
    "S/S Government consumption to GDP ratio" rw_ss_ncg_to_ngdp
    "S/S General import tariff rate" rw_ss_trm

    "Response in government consumption to government debt" rw_tau_cg
    "Response in type 1 lump-sum tax rate to government debt" rw_tau_txls1
    "Autoregression in government consumption" rw_rho_cg
    "Autoregression in type 1 lump-sum tax rate" rw_rho_txls2
    "Autoregression in type 2 lump-sum tax rate" rw_rho_txls1
    "Autoregression in generate import tariff rate" rw_rho_trm
...
!variables(:rw_open)
    ...
    "Price of non-commodity imports, Free on board at destinatin point" rw_fob_pmm
    "Import content of non-commodity exports" rw_mxx
    "Intermediate import inputs into domestic production" rw_my
    ...
    "Net foreign assets to GDP Ratio" rw_nfa_to_ngdp
    "Effective interest rate and valuation on NFA" rw_rnfa
    "Checksum for net foreign assets to GDP ratio" rw_nfa_to_ngdp_checksum
    "Nominal exchange rate" rw_e
    "Expectations of nominal exchange rate" rw_exp_e
    "Nominal Exchange Rate, Rate of Change" rw_roc_e
    ...
    "Price of non-commodity exports, Reference currency" rw_pxx_rcy
    "Value of non-commodity exports, Reference currency" rw_nxx_rcy
...
!parameters(:rw_open :rw_steady)

    "Import intensity of non-commodity exports" rw_alpha
    "Acceleration in exportable productivity" rw_gamma_xx
    "Elasticity of household risk function wrt NFA" rw_theta_1
    "Curvature of household risk function wrt NFA" rw_theta_2
    "Share of local commodity production in world commodity production" rw_lambda
    "S/S Autonomous component in country credit risk" rw_ss_zh_aut


!parameters(:rw_open :rw_transition)

    "A/R Autonomous component in country credit risk" rw_rho_zh_aut
    "Weight on model-consistent expectations in exchange rate" rw_zeta_e
    "Adjustment cost parameter in stage t-3 production" rw_xi_y3
...
!equations(:rw_open)
    ...
    "Demand for re-exports"
    rw_alpha * rw_pxx * rw_xx = rw_pmm * rw_mxx;


    "Balance of payments transactions"
    rw_nfa_to_ngdp * rw_ngdp = + rw_rnfa * rw_nfa_to_ngdp{-1}*rw_ngdp{-1} + rw_bpfeq_to_ngdp * rw_ngdp + rw_bpceq_to_ngdp * rw_ngdp + rw_pxx * rw_xx - rw_fob_pmm * rw_mm + rw_pq * (rw_xq - rw_mq) + rw_nfa_to_ngdp_checksum * rw_ngdp;
...
!equations
    ...
    0 = + us_nfa_to_ngdp * us_ngdp / us_e + ea_nfa_to_ngdp * ea_ngdp / ea_e + cn_nfa_to_ngdp * cn_ngdp / cn_e + rw_nfa_to_ngdp * rw_ngdp / rw_e ;
        ...
        "Interest parity"
        ea_r = us_r * ea_exp_e / ea_e * exp(ea_shk_e);
        ...
        "Interest parity"
        cn_r = us_r * cn_exp_e / cn_e * exp(cn_shk_e);
        ...
        "Interest parity"
        rw_r = us_r * rw_exp_e / rw_e * exp(rw_shk_e);
    ...
    !variables
        "Nominal GDP, Reference currency" us_ngdp_rcy
    ...
    !variables
        "Nominal GDP, Reference currency" ea_ngdp_rcy
    ...
    !variables
        "Nominal GDP, Reference currency" cn_ngdp_rcy
    ...
    !variables
        "Nominal GDP, Reference currency" rw_ngdp_rcy
...
!variables
            ...
            "Share of imports from [cn] in total imports to [?]"
            us_mm_sh_cn
        
            "Share of imports from [ea] in total imports to [?]"
            us_mm_sh_ea
        
            "Share of imports from [rw] in total imports to [?]"
            us_mm_sh_rw
            ...
            "Share of imports from [us] in total imports to [?]"
            ea_mm_sh_us
...
!parameters
        ...
        us_xi_mm
            ...
            "Share of imports from [cn] in total [us] imports"
            us_omega_cn
        
            "Share of imports from [ea] in total [us] imports"
            us_omega_ea
        
            "Share of imports from [rw] in total [us] imports"
            us_omega_rw
        ...
        ea_xi_mm
            ...
            "Share of imports from [cn] in total [ea] imports"
            ea_omega_cn
        
            "Share of imports from [rw] in total [ea] imports"
            ea_omega_rw
        
            "Share of imports from [us] in total [ea] imports"
            ea_omega_us
        ...
        cn_xi_mm
            ...
            "Share of imports from [ea] in total [cn] imports"
            cn_omega_ea
        
            "Share of imports from [rw] in total [cn] imports"
            cn_omega_rw
        
            "Share of imports from [us] in total [cn] imports"
            cn_omega_us
        ...
        rw_xi_mm
            ...
            "Share of imports from [cn] in total [rw] imports"
            rw_omega_cn
        
            "Share of imports from [ea] in total [rw] imports"
            rw_omega_ea
        
            "Share of imports from [us] in total [rw] imports"
            rw_omega_us
...
!equations
        ...
        !parameters
            "S/S Area specific import tariff rate" us_ss_trm_cn
            "S/S Autoregression in area specific import tariff rate" us_rho_trm_cn
        ...
        !parameters
            "S/S Area specific import tariff rate" us_ss_trm_ea
            "S/S Autoregression in area specific import tariff rate" us_rho_trm_ea
        ...
        !parameters
            "S/S Area specific import tariff rate" us_ss_trm_rw
            "S/S Autoregression in area specific import tariff rate" us_rho_trm_rw
        ...
        !parameters
            "S/S Area specific import tariff rate" ea_ss_trm_cn
            "S/S Autoregression in area specific import tariff rate" ea_rho_trm_cn
        ...
        !parameters
            "S/S Area specific import tariff rate" ea_ss_trm_rw
            "S/S Autoregression in area specific import tariff rate" ea_rho_trm_rw
        ...
        !parameters
            "S/S Area specific import tariff rate" ea_ss_trm_us
            "S/S Autoregression in area specific import tariff rate" ea_rho_trm_us
        ...
        !parameters
            "S/S Area specific import tariff rate" cn_ss_trm_ea
            "S/S Autoregression in area specific import tariff rate" cn_rho_trm_ea
        ...
        !parameters
            "S/S Area specific import tariff rate" cn_ss_trm_rw
            "S/S Autoregression in area specific import tariff rate" cn_rho_trm_rw
        ...
        !parameters
            "S/S Area specific import tariff rate" cn_ss_trm_us
            "S/S Autoregression in area specific import tariff rate" cn_rho_trm_us
        ...
        !parameters
            "S/S Area specific import tariff rate" rw_ss_trm_cn
            "S/S Autoregression in area specific import tariff rate" rw_rho_trm_cn
        ...
        !parameters
            "S/S Area specific import tariff rate" rw_ss_trm_ea
            "S/S Autoregression in area specific import tariff rate" rw_rho_trm_ea
        ...
        !parameters
            "S/S Area specific import tariff rate" rw_ss_trm_us
            "S/S Autoregression in area specific import tariff rate" rw_rho_trm_us
...
!parameters
        ...
        "Forward capital adjustment cost parameter" us_xi_k
        
            "Corporate equity portfolio share" us_phi_us
        
            "Corporate equity portfolio share" us_phi_ea
        
            "Corporate equity portfolio share" us_phi_cn
        
            "Corporate equity portfolio share" us_phi_rw
        
    
        "Forward capital adjustment cost parameter" ea_xi_k
        
            "Corporate equity portfolio share" ea_phi_us
        
            "Corporate equity portfolio share" ea_phi_ea
        
            "Corporate equity portfolio share" ea_phi_cn
        
            "Corporate equity portfolio share" ea_phi_rw
        
    
        "Forward capital adjustment cost parameter" cn_xi_k
        
            "Corporate equity portfolio share" cn_phi_us
        
            "Corporate equity portfolio share" cn_phi_ea
        
            "Corporate equity portfolio share" cn_phi_cn
        
            "Corporate equity portfolio share" cn_phi_rw
        
    
        "Forward capital adjustment cost parameter" rw_xi_k
        
            "Corporate equity portfolio share" rw_phi_us
        
            "Corporate equity portfolio share" rw_phi_ea
        
            "Corporate equity portfolio share" rw_phi_cn
        
            "Corporate equity portfolio share" rw_phi_rw
