% [IrisToolbox] autogenerated preparsed model source code 06-Sep-2023 18:20:44
%
...
!equations(:households)
    ...
    log(us_w_to_pc) = + 0.5 * [log(us_w_to_pc{-1}) + log(&us_roc_w/&us_roc_pc)] + (1-0.5) * log(us_ww/us_pc) !! us_w = us_ww;
...
!equations(:households)
    ...
    log(ea_w_to_pc) = + 0.5 * [log(ea_w_to_pc{-1}) + log(&ea_roc_w/&ea_roc_pc)] + (1-0.5) * log(ea_ww/ea_pc) !! ea_w = ea_ww;
...
!equations(:households)
    ...
    log(cn_w_to_pc) = + 0.5 * [log(cn_w_to_pc{-1}) + log(&cn_roc_w/&cn_roc_pc)] + (1-0.5) * log(cn_ww/cn_pc) !! cn_w = cn_ww;
...
!equations(:households)
    ...
    log(rw_w_to_pc) = + 0.5 * [log(rw_w_to_pc{-1}) + log(&rw_roc_w/&rw_roc_pc)] + (1-0.5) * log(rw_ww/rw_pc) !! rw_w = rw_ww;
