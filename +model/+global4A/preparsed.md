% [IrisToolbox] autogenerated preparsed model source code 01-Feb-2024 08:44:18
%
...
!postprocessor(:demography)
    ...
    us_nw_to_nn = us_nw / us_nn;
    us_nf_to_nw = us_nf / us_nw;
...
!variables(:households)
    ...
    "Auxiliary calibration variable" us_ss_nh_to_ref_nh
    ...
    "Inflation expectations" us_E_roc_pc
...
!parameters(:households :steady)
    ...
    "Inverse short-run elasticity of labor supply !! $\eta$" us_eta_short
    "Inverse long-run elasticity of labor supply !! $\eta$" us_eta_long
    "Utility location parameter of labor supply !! $\eta_0$" us_eta0
    "Reference level of long-run per-worker labor supply !! $\mathit{nh}_\mathrm{ref}$" us_ref_nh
...
!parameters(:households :dynamic)
    ...
    "A/R in long-run labor supply" us_rho_nh
...
!equations(:households)
    ...
    us_vh * us_ww = us_eta0 * (us_nh/&us_nh)^us_eta_short * (us_ss_nh_to_ref_nh)^us_eta_long !! us_vh * us_ww = us_eta0 * (us_ss_nh_to_ref_nh)^us_eta_long;

    "Auxiliary calibration equation for long-run labor supply"
    us_ss_nh_to_ref_nh = + us_rho_nh * us_ss_nh_to_ref_nh{-1} + (1 - us_rho_nh) * &us_ss_nh_to_ref_nh !! us_ss_nh_to_ref_nh = &us_nh / us_ref_nh;
    ...
    us_E_roc_pc = us_roc_pc{+1};


## us_Postprocessing us_equations us_outside us_model
...
!postprocessor(:demography)
    ...
    ea_nw_to_nn = ea_nw / ea_nn;
    ea_nf_to_nw = ea_nf / ea_nw;
...
!variables(:households)
    ...
    "Auxiliary calibration variable" ea_ss_nh_to_ref_nh
    ...
    "Inflation expectations" ea_E_roc_pc
...
!parameters(:households :steady)
    ...
    "Inverse short-run elasticity of labor supply !! $\eta$" ea_eta_short
    "Inverse long-run elasticity of labor supply !! $\eta$" ea_eta_long
    "Utility location parameter of labor supply !! $\eta_0$" ea_eta0
    "Reference level of long-run per-worker labor supply !! $\mathit{nh}_\mathrm{ref}$" ea_ref_nh
    "Intercept in current wealth utility !! $\nu_0$" ea_nu_0
    "Slope of current wealth utility !! $\nu_1$" ea_nu_1
...
!parameters(:households :dynamic)
    ...
    "A/R in long-run labor supply" ea_rho_nh
...
!equations(:households)
    ...
    "Short-run and long-run labor supply"
    ea_vh * ea_ww = ea_eta0 * (ea_nh/&ea_nh)^ea_eta_short * (ea_ss_nh_to_ref_nh)^ea_eta_long !! ea_vh * ea_ww = ea_eta0 * (ea_ss_nh_to_ref_nh)^ea_eta_long;

    "Auxiliary calibration equation for long-run labor supply"
    ea_ss_nh_to_ref_nh = + ea_rho_nh * ea_ss_nh_to_ref_nh{-1} + (1 - ea_rho_nh) * &ea_ss_nh_to_ref_nh !! ea_ss_nh_to_ref_nh = &ea_nh / ea_ref_nh;
    ...
    ea_E_roc_pc = ea_roc_pc{+1};


## ea_Postprocessing ea_equations ea_outside ea_model
...
!postprocessor(:demography)
    ...
    cn_nw_to_nn = cn_nw / cn_nn;
    cn_nf_to_nw = cn_nf / cn_nw;
...
!variables(:households)
    ...
    "Auxiliary calibration variable" cn_ss_nh_to_ref_nh
    ...
    "Inflation expectations" cn_E_roc_pc
...
!parameters(:households :steady)
    ...
    "Inverse short-run elasticity of labor supply !! $\eta$" cn_eta_short
    "Inverse long-run elasticity of labor supply !! $\eta$" cn_eta_long
    "Utility location parameter of labor supply !! $\eta_0$" cn_eta0
    "Reference level of long-run per-worker labor supply !! $\mathit{nh}_\mathrm{ref}$" cn_ref_nh
    "Intercept in current wealth utility !! $\nu_0$" cn_nu_0
    "Slope of current wealth utility !! $\nu_1$" cn_nu_1
...
!parameters(:households :dynamic)
    ...
    "A/R in long-run labor supply" cn_rho_nh
...
!equations(:households)
    ...
    "Short-run and long-run labor supply"
    cn_vh * cn_ww = cn_eta0 * (cn_nh/&cn_nh)^cn_eta_short * (cn_ss_nh_to_ref_nh)^cn_eta_long !! cn_vh * cn_ww = cn_eta0 * (cn_ss_nh_to_ref_nh)^cn_eta_long;

    "Auxiliary calibration equation for long-run labor supply"
    cn_ss_nh_to_ref_nh = + cn_rho_nh * cn_ss_nh_to_ref_nh{-1} + (1 - cn_rho_nh) * &cn_ss_nh_to_ref_nh !! cn_ss_nh_to_ref_nh = &cn_nh / cn_ref_nh;
    ...
    cn_E_roc_pc = cn_roc_pc{+1};


## cn_Postprocessing cn_equations cn_outside cn_model
...
!postprocessor(:demography)
    ...
    rw_nw_to_nn = rw_nw / rw_nn;
    rw_nf_to_nw = rw_nf / rw_nw;
...
!variables(:households)
    ...
    "Auxiliary calibration variable" rw_ss_nh_to_ref_nh
...
!parameters(:households :steady)
    ...
    "Inverse short-run elasticity of labor supply !! $\eta$" rw_eta_short
    "Inverse long-run elasticity of labor supply !! $\eta$" rw_eta_long
    "Utility location parameter of labor supply !! $\eta_0$" rw_eta0
    "Reference level of long-run per-worker labor supply !! $\mathit{nh}_\mathrm{ref}$" rw_ref_nh
    "Intercept in current wealth utility !! $\nu_0$" rw_nu_0
    "Slope of current wealth utility !! $\nu_1$" rw_nu_1
...
!parameters(:households :dynamic)
    ...
    "Adjustment cost in wage setting" rw_xi_w
    "A/R in long-run labor supply" rw_rho_nh
...
!equations(:households)
    ...
    "Short-run and long-run labor supply"
    rw_vh * rw_ww = rw_eta0 * (rw_nh/&rw_nh)^rw_eta_short * (rw_ss_nh_to_ref_nh)^rw_eta_long !! rw_vh * rw_ww = rw_eta0 * (rw_ss_nh_to_ref_nh)^rw_eta_long;

    "Auxiliary calibration equation for long-run labor supply"
    rw_ss_nh_to_ref_nh = + rw_rho_nh * rw_ss_nh_to_ref_nh{-1} + (1 - rw_rho_nh) * &rw_ss_nh_to_ref_nh !! rw_ss_nh_to_ref_nh = &rw_nh / rw_ref_nh;

    "Real consumer wage rate"
    rw_w_to_pc = rw_w / rw_pc;

    "Wage rigidities"
    log(rw_ww) - log(rw_w) = + rw_xi_w*(log(rw_roc_w) - log(rw_roc_w{-1})) - rw_beta/&rw_roc_w*rw_xi_w*(log(rw_roc_w{+1}) - log(rw_roc_w)) + rw_shk_w !! rw_w = rw_ww;
    ...
    rw_E_roc_pc = rw_roc_pc{+1};


## rw_Postprocessing rw_equations rw_outside rw_model
...
!parameters(:production :steady)
    ...
    "Share of commodity inputs in stage T-1 production !! $\gamma_mq$" rw_gamma_mq
...
!equations(:production)
    ...
    rw_py1 = rw_alpha_y2*rw_py2 + rw_alpha_mq*rw_pq !! rw_y1 = rw_a_y1 * (rw_y2/(1-rw_gamma_mq))^(1-rw_gamma_mq) * (rw_mq/rw_gamma_mq)^rw_gamma_mq ;

    "Demand for Y2 inputs"
    rw_alpha_y2 * rw_y1 = rw_y2;

    "Demand for commodity inputs"
    rw_alpha_mq * rw_y1 = rw_mq;

    "Input share parameter for Y2"
    rw_alpha_y2 = + rw_rho_alpha * rw_alpha_y2{-1} + (1 - rw_rho_alpha) * (1-rw_gamma_mq)*(&rw_py1/&rw_py2) !! rw_alpha_y2 = (1-rw_gamma_mq)*(rw_py1/rw_py2) ;

    "Input share parameter for commodity"
    rw_alpha_mq = + rw_rho_alpha * rw_alpha_mq{-1} + (1 - rw_rho_alpha) * rw_gamma_mq*(&rw_py1/&rw_pq) !! rw_alpha_mq = rw_gamma_mq*(rw_py1/rw_pq) ;
...
!parameters(:open :steady)
    ...
    "Import intensity of non-commodity exports !! $\gamma_\mathit{mxx}$" rw_gamma_mxx
...
!equations(:open)
    ...
    "Non-commodity export production function"
    rw_xx = rw_ar^rw_gamma_xx * (rw_yxx/(1-rw_gamma_mxx))^(1-rw_gamma_mxx) * (rw_mxx/rw_gamma_mxx)^rw_gamma_mxx;

    "Demand for local content in export production"
    (1-rw_gamma_mxx) * rw_pxx * rw_xx = rw_py * rw_yxx;

    "Demand for re-exports"
    rw_gamma_mxx * rw_pxx * rw_xx = (rw_pmm * gg_dmm) * rw_mxx;
